  ...
  try {
    ...
    if (authentication.grantType === 'authorization_code') {
      ...
      if (authentication.useDefaultBrowser) {
        timeline.push(
          logEntry(`Opening default browser for authorization: ${authCodeUrl.origin}${authCodeUrl.pathname}`),
        );
        ...
        timeline.push(logEntry(`Opening authorization window: ${authCodeUrl.origin}${authCodeUrl.pathname}`));
        try {
          redirectedTo = await authorizeUserInWindow({
      ...
      console.log('[oauth2] Detected redirect ' + redirectedTo);
      timeline.push(logEntry('Authorization redirect detected, exchanging code for token'));
    ...
    const token = await services.oAuth2Token.update(old, transformNewAccessTokenToOauthModel(tokenData));
    return { token, timeline };
  } catch (err) {
    if (authentication.useDefaultBrowser) {
      hideOAuthAuthorizationModal();
